...
#Import TAD data
level1 <- read_tsv("./iconhic_JS_refJS_merged_maskXII_XXI_Y_pairtools_spectraltad_25kb_Level1.bed", col_names = c("chr", "start", "end", "level")) %>%
  pivot_longer(cols = c("start", "end"), names_to = "position", values_to = "bp")  %>%
  unique()

level2 <- read_tsv("./iconhic_JS_refJS_merged_maskXII_XXI_Y_pairtools_spectraltad_25kb_Level2_unique.bed", col_names = c("chr", "start", "end", "level"))%>%
  pivot_longer(cols = c("start", "end"), names_to = "position", values_to = "bp") %>%
  unique()
...
mean(read_tsv("./iconhic_JS_refJS_merged_maskXII_XXI_Y_pairtools_spectraltad_25kb_Level1.bed", col_names = c("chr", "start", "end", "level")) %>%
       dplyr::mutate(length = end - start) %>%
       pull(length))
median(read_tsv("./iconhic_JS_refJS_merged_maskXII_XXI_Y_pairtools_spectraltad_25kb_Level1.bed", col_names = c("chr", "start", "end", "level")) %>%
         dplyr::mutate(length = end - start) %>%
         pull(length))

mean(read_tsv("./iconhic_JS_refJS_merged_maskXII_XXI_Y_pairtools_spectraltad_25kb_Level2_unique.bed", col_names = c("chr", "start", "end", "level")) %>%
       dplyr::mutate(length = end - start) %>%
       pull(length))
median(read_tsv("./iconhic_JS_refJS_merged_maskXII_XXI_Y_pairtools_spectraltad_25kb_Level2_unique.bed", col_names = c("chr", "start", "end", "level")) %>%
         dplyr::mutate(length = end - start) %>%
         pull(length))
