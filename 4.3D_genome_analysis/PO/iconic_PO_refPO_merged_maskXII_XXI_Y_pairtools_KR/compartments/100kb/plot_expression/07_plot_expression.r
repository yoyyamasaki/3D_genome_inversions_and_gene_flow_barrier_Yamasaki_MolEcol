...
#File path
tpm_data <- "~/HDD/yo_analysis/HDD4_20230223/stickleback/HiC/chromatin_analysis/PO_hifi/RNAseq/PO_male_merged_maskXII_XXI_Y/RNAseq_analysis/counts_tidy_tpm_filtered.txt"
...
PO_gff_gene_table_A <- read_tsv(A_compartment_table, skip = 1, col_names = bed_col) %>%
  dplyr::filter(chr != "chrXIX")
PO_gff_gene_table_B <- read_tsv(B_compartment_table, skip = 1, col_names = bed_col) %>%
  dplyr::filter(chr != "chrXIX")

overlapped_genes <- intersect(PO_gff_gene_table_A$Geneid, PO_gff_gene_table_B$Geneid)

#Read TPM data
counts_tidy_tpm_filtered <- read_tsv(tpm_data)

#Merge compartment and TPM data
A_tpm <- left_join(PO_gff_gene_table_A, counts_tidy_tpm_filtered)
A_tpm <- A_tpm %>% dplyr::mutate(compartment = rep("A", nrow(A_tpm)))

B_tpm <- left_join(PO_gff_gene_table_B, counts_tidy_tpm_filtered)
...
for(j in tissue){
  ...
  plot_AB_expression <- ggplot(drop_na(compartment_tpm_tissue), aes(x = compartment, y = tpm_mean + 1))+
...
#Plot pituitary separately with chaging y axis height
j="pit"
...
plot_AB_expression <- ggplot(compartment_tpm_tissue, aes(x = compartment, y = tpm_mean + 1))+
  ...
  geom_signif(comparisons = list(c("A", "B")),
              test = "brunner.munzel.test", na.rm = FALSE, map_signif_level = F, col = "red") +
  scale_y_log10(limits = c(1, max(compartment_tpm_tissue$tpm_mean) + 300000)) +
  scale_fill_manual(values = c("red3", "cyan")) +
  ggtitle(paste0(j, "_all")) +
  theme(panel.background = element_blank(),
        panel.border = element_rect(fill = NA, colour = "black", size = 1,
...
ggsave(paste0(j, "/expression_", j, "_compartment_all.eps"), plot_AB_expression, height = 3, width = 3)
