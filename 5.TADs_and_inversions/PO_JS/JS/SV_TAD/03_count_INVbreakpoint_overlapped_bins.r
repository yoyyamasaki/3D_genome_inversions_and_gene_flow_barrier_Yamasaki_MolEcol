...
################################################################################
#Read files
inversion <- read_tsv("./JS/inversion_JS_PObased.bed", col_names = c("chr", "start", "end"))
...
genome <- read_tsv("~/db/JS_male_merged_mask_chrXII_chrXXI_chrYXI_20240517/JS_male_merged.v20240517.maskXII_XXI_Y.fa.order.genome", col_names = c("chr", "length")) %>%
...
inversion_ordered <- inversion_ordered %>%
  dplyr::mutate(size = end - start) %>%
  dplyr::mutate(inversionID = paste0("INV_", seq(1, nrow(inversion_ordered))))
...
TAD_boundaries <- bind_rows(TAD1, TAD2) %>%
  ...
  dplyr::mutate(start = bp - 25000 - 1, end = bp + 25000 - 1) %>%
  dplyr::select(chr, start, end) %>%
  unique()
...
################################################################################
# For all inversions
##Make bed of inversion breakpoint positions
