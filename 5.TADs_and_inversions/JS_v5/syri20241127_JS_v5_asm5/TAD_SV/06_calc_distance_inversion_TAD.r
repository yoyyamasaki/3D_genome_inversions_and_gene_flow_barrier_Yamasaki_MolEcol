...
################################################################################
#Read files
inversion <- read_tsv("../syri20241127_JS_v5_asm5syri.INV.bed", col_names = c("chr", "start", "end"))
...
inversion_ordered <- inversion_ordered %>%
  dplyr::mutate(size = end - start) %>%
  dplyr::mutate(inversionID = paste0("INVJS_", seq(1, nrow(inversion_ordered))))
...
Inversion_dist <- ggplot(inversion_ordered) +
  ...
  ggtitle("Size and number of inversions: JS_v5") +
...
ggsave("INV_dist_JS_v5.eps", Inversion_dist, width =5, height = 5)

#Extract 25kb < inversions
inversion_25kb <- inversion_ordered %>%
  dplyr::filter(size >= 25000)
write_tsv(inversion_25kb, "./syri20240917_JS_v5_asm5syri.25kb.INV.bed", col_names = F)
...
write_tsv(inversion_25kb_breakpoints_merged, "./syri20240917_JS_v5_asm5syri.25kb.INV_breakpoint.bed", col_names = F)
...
for(i in 1:nrow(inversion_25kb)){
  ...
  lear_distance <- find_closest_to_zero(pull(inversion_25kb[i, "end"]) - unique(c(TAD_tmp$start, TAD_tmp$end)))
  ...
  inversion_TAD_dist_tmp <- inversion_25kb[i, ] %>%
    dplyr::mutate(front_distance_JS = front_distance,
                  ...
                  lear_distance_JS = lear_distance,
                  lear_bin_JS = lear_distance/25000)
...
write_tsv(inversion_TAD_distance, "./inversion_JS_v5_asm5_TAD_JS_dist.txt")
write_tsv(TAD_overlap, "./inversion_asm5_overlapped_TAD_JS.txt")
...
inversion_TAD_distance_longer <- inversion_TAD_distance %>%
  pivot_longer(cols = c("front_bin_JS", "lear_bin_JS"), names_to = "front_rear", values_to = "N_bin") 
...
inversion_TAD_distance_longer_orthologous <- inversion_TAD_distance_longer %>%
  dplyr::filter(!(inversionID %in% c("INVJS_3", "INVJS_18")))
...
INV_TAD_dist_hist <- ggplot(tibble(breaks = inversion_TAD_distance_hist$breaks[-1], counts = inversion_TAD_distance_hist$counts)) +
  ...
  scale_x_continuous(breaks = c(1, seq(5, 50, 5)), limits = c(0.5, 21)) +
...
ggsave("INV_TAD_distans_bin_JS_v5.eps", INV_TAD_dist_hist, width =5, height = 5)
...
for(i in 1:nrow(inversion_ordered)){
  ...
  if(nrow(TAD_tmp) > 0){
    ...
    lear_distance <- find_closest_to_zero(pull(inversion_ordered[i, "end"]) - unique(c(TAD_tmp$start, TAD_tmp$end)))
...
write_tsv(inversion_TAD_distance_all, "./inversion_JS_v5_asm5_TAD_JS_dist_all.txt")
write_tsv(TAD_overlap_all, "./inversion_asm5_overlapped_TAD_JS_all.txt")
...
INV_TAD_dist_all_hist <- ggplot(tibble(breaks = inversion_TAD_distance_all_hist$breaks[-1], counts = inversion_TAD_distance_all_hist$counts)) +
  geom_col(aes(x = breaks, y = counts)) + 
  ggtitle("Distance between breakpoint and TAD boundary") +
  scale_x_continuous(breaks = c(1, seq(5, 75, 5)), limits = c(0.5, 21)) +
...
ggsave("INV_TAD_distans_bin_JS_v5_all.eps", INV_TAD_dist_all_hist, width =5, height = 5)
