...
################################################################################
#Read files
inversion <- read_tsv("../syri20241127_PO_v5_asm5syri.INV.bed", col_names = c("chr", "start", "end"))
...
Inversion_dist <- ggplot(inversion_ordered) +
  ...
  ggtitle("Size and number of inversions: PO_v5") +
...
ggsave("INV_dist_PO_v5.eps", Inversion_dist, width =5, height = 5)

#Extract 25kb < inversions
inversion_25kb <- inversion_ordered %>%
  dplyr::filter(size >= 25000)
write_tsv(inversion_25kb, "./syri20240917_PO_v5_asm5syri.25kb.INV.bed", col_names = F)
...
write_tsv(inversion_25kb_breakpoints_merged, "./syri20240917_PO_v5_asm5syri.25kb.INV_breakpoint.bed", col_names = F)
...
write_tsv(inversion_TAD_distance, "./inversion_PO_v5_asm5_TAD_PO_dist.txt")
...
ggsave("INV_TAD_distans_bin_PO_v5.eps", INV_TAD_dist_hist, width =5, height = 5)
...
for(i in 1:nrow(inversion_ordered)){
  #Extract inversion overlapped TADs    ssss  
...
write_tsv(inversion_TAD_distance_all, "./inversion_PO_v5_asm5_TAD_v5_dist_all.txt")
...
inversion_TAD_distance_all_hist <- hist(inversion_TAD_distance_longer_all$N_bin, plot = F, breaks = seq(0, 52))
...
INV_TAD_dist_all_hist <- ggplot(tibble(breaks = inversion_TAD_distance_all_hist$breaks[-1], counts = inversion_TAD_distance_all_hist$counts)) +
  geom_col(aes(x = breaks, y = counts)) + 
  ggtitle("Distance between breakpoint and TAD boundary") +
  scale_x_continuous(breaks = c(1, seq(5, 50, 5)), limits = c(0.5, 20)) +
...
ggsave("INV_TAD_distans_bin_PO_v5_all.eps", INV_TAD_dist_all_hist, width =5, height = 5)
